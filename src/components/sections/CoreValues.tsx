...
const icons: ReactNode[] = [
  (
      <svg key="integriteit" width="32" height="32" viewBox="0 0 49 49" fill="none">
        <path
  ...
  (
      <svg key="excellentie" width="32" height="32" viewBox="0 0 49 49" fill="none">
        <path
  ...
  (
      <svg key="clientgericht" width="32" height="32" viewBox="0 0 56 56" fill="none">
        <path
  ...
  (
      <svg key="daadkrachtig" width="32" height="32" viewBox="0 0 44 44" fill="none">
        <path
...
export default async function CoreValues() {
  const t = await getTranslations("coreValues");
  const values: Value[] = (
    t.raw("values") as { title: string; description: string }[]
  ).map((value, index) => ({ ...value, icon: icons[index] }));
